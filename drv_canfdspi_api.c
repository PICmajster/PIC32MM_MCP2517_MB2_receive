...
void DRV_CANFDSPI_MY_Configure(void)
{
   ...
   /*FIFO 1: Receive FIFO; 1 messages, 8 byte maximum payload, time stamping disabled*/
    CAN_RX_FIFO_CONFIG rxfConfig;
    rxfConfig.FifoSize = 0; //0 = 1 message
    rxfConfig.PayLoadSize = CAN_PLSIZE_8;
    rxfConfig.RxTimeStampEnable = 0;
    DRV_CANFDSPI_ReceiveChannelConfigure(CAN_FIFO_CH1, &rxfConfig);
    
    /*FILTER config*/
    DRV_CANFDSPI_FilterDisable(CAN_FILTER0);
//    CAN_FILTEROBJ_ID fObj ;
//    fObj.SID = 0x300;
//    fObj.SID11 = 0;
//    fObj.EID = 0;
////    
//    DRV_CANFDSPI_FilterObjectConfigure(CAN_FILTER0, &fObj);
    
    CAN_MASKOBJ_ID mObj;
    mObj.MSID = 0x0000 ; //0 represents acceptance
    mObj.MSID11 = 1;
    mObj.MEID = 0; //0 not match EID bit
    mObj.MIDE = 0; //0 not match IDE bit
    
    DRV_CANFDSPI_FilterMaskConfigure(CAN_FILTER0, &mObj);
    DRV_CANFDSPI_FilterToFifoLink(CAN_FILTER0, CAN_FIFO_CH1, true);
    
    
    /*Interrupt RX eneable*/
    DRV_CANFDSPI_ModuleEventEnable(CAN_RX_EVENT); //if TX and RX , (CAN_RX_EVENT | CAN_TX_EVENT)
